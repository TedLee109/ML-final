...
def perform_tuning(
    ...
    for global_step, batch in enumerate(itertools.islice(itertools.cycle(dataloader), num_steps), start=1):
        ...
        progress_bar.update(1)
        progress_bar.set_postfix(loss=loss.item(), lr=lr_scheduler_lora.get_last_lr()[0])

        if global_step % save_steps == 0 or global_step == num_steps:
            save_all(
                unet,
                text_encoder,
                placeholder_token_ids=placeholder_token_ids,
                placeholder_tokens=placeholder_tokens,
                save_path=os.path.join(
                    save_path, f"step_{global_step if global_step != num_steps else out_name}.safetensors"
                ),
                target_replace_module_text=lora_clip_target_modules,
                target_replace_module_unet=lora_unet_target_modules,
            )

            lora_unet_moved = torch.tensor(
                list(itertools.chain(*inspect_lora(unet).values()))
            ).mean().item()
            lora_clip_moved = torch.tensor(
                list(itertools.chain(*inspect_lora(text_encoder).values()))
            ).mean().item()
            print(f"LORA Unet Moved: {lora_unet_moved}")
            print(f"LORA CLIP Moved: {lora_clip_moved}")
...
def train(
    ...
    output_dir: str,
    train_text_encoder: bool = True, #True
    pretrained_vae_name_or_path: str = None,
    revision: Optional[str] = None,
    perform_inversion: bool = True,#True
